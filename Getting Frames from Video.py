...
def video_to_frames(input_loc):
    ...
    output_loc += "Frames of "+name_of_video
    ...
    #If everything is alright==>
        ...
        while cap.isOpened():
            ...
            if count % ((int(fps))*(frequency)) == 0:
                try:
                    cv2.imwrite(output_loc + "\\Frame_No%s.jpg" % count, frame)
    ...
    for photos in os.listdir(output_loc):
        ...
        if (start_time-1)*int(fps)>int(photos[8:-4]):
...
if __name__ == "__main__":
    ...
    input_loc = "C:\\Users\\User\\Desktop\\"
